...
public class ParallelDataZipper {
    ...
    public int getStreamNumberByTimestamp(long j) {
        ...
        String str = TAG;
        StringBuilder sb = new StringBuilder();
        sb.append("getStreamNumberByTimestamp: getStreamNumberByTimestamp returned an error result with timestamp: ");
        sb.append(j);
    ...
    public synchronized void join(final Image image, final int i) {
        ...
        this.mHandler.post(new Runnable() {
            public void run() {
                ...
                String access$300 = ParallelDataZipper.TAG;
                StringBuilder sb = new StringBuilder();
    ...
    public synchronized void join(@NonNull final ICustomCaptureResult iCustomCaptureResult, final boolean z) {
        ...
        this.mHandler.post(new Runnable() {
            public void run() {
                ...
                int access$100 = ParallelDataZipper.this.getStreamNumberByTimestamp(timeStamp);
                CaptureDataBean captureDataBean = (CaptureDataBean) ParallelDataZipper.this.mCaptureDataBeanArray.get(timeStamp);
                if (captureDataBean == null) {
                    captureDataBean = new CaptureDataBean(access$100);
                    ParallelDataZipper.this.mCaptureDataBeanArray.append(timeStamp, captureDataBean);
                }
                if (captureDataBean.getStreamNum() == 0 && access$100 != 0) {
                    captureDataBean.setStreamNum(access$100);
                }
                captureDataBean.setCaptureResult(iCustomCaptureResult, z);
                String access$300 = ParallelDataZipper.TAG;
                StringBuilder sb = new StringBuilder();
                sb.append("setResult: timestamp=");
                sb.append(timeStamp);
    ...
    public synchronized void startTask(@NonNull CaptureData captureData) {
        String str = TAG;
        StringBuilder sb = new StringBuilder();
        sb.append("startTask: ");
