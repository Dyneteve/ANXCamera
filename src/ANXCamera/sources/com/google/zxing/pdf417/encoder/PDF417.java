...
public final class PDF417 {
    ...
    public void generateBarcodeLogic(String msg, int errorCorrectionLevel) throws WriterException {
        ...
        String highLevel = PDF417HighLevelEncoder.encodeHighLevel(msg, pdf417.compaction, pdf417.encoding);
        int sourceCodeWords = highLevel.length();
        int[] dimension = pdf417.determineDimensions(sourceCodeWords, errorCorrectionCodeWords);
        ...
        if (sourceCodeWords + errorCorrectionCodeWords + 1 <= 929) {
            ...
            StringBuilder sb2 = new StringBuilder(String.valueOf(dataCodewords));
            sb2.append(ec);
            String fullCodewords = sb2.toString();
            pdf417.barcodeMatrix = new BarcodeMatrix(rows, cols);
            BarcodeMatrix barcodeMatrix2 = pdf417.barcodeMatrix;
            String str = ec;
            String str2 = dataCodewords;
            StringBuilder sb3 = sb;
            pdf417.encodeLowLevel(fullCodewords, cols, rows, errorCorrectionLevel, barcodeMatrix2);
            return;
        }
        StringBuilder sb4 = new StringBuilder("Encoded message contains to many code words, message to big (");
        sb4.append(msg.length());
        sb4.append(" bytes)");
    ...
    private int[] determineDimensions(int sourceCodeWords, int errorCorrectionCodeWords) throws WriterException {
        ...
        for (int cols = this.minCols; cols <= this.maxCols; cols++) {
            ...
            if (rows <= this.maxRows) {
                float newRatio = (((float) ((17 * cols) + 69)) * DEFAULT_MODULE_WIDTH) / (((float) rows) * 2.0f);
