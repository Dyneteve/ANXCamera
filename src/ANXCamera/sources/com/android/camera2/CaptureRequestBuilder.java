...
public class CaptureRequestBuilder {
    ...
    static void applyExposureCompensation(Builder builder, int i, CameraCapabilities cameraCapabilities, CameraConfigs cameraConfigs) {
        if (builder != null) {
            ...
            if (b.jr() && ModuleManager.isManualModule() && i == 1 && cameraConfigs.getISO() == 0 && cameraConfigs.getExposureTime() > MAX_REALTIME_EXPOSURE_TIME) {
    ...
    static void applyIso(Builder builder, int i, CameraCapabilities cameraCapabilities, CameraConfigs cameraConfigs) {
        if (builder != null) {
            ...
            if (b.jr() && i == 1 && iso > 0 && cameraConfigs.getExposureTime() > MAX_REALTIME_EXPOSURE_TIME) {
    ...
    static void applyPortraitLighting(Builder builder, int i, CameraCapabilities cameraCapabilities, CameraConfigs cameraConfigs) {
        if (builder != null) {
            if ((i == 3 || DataRepository.dataItemFeature().gX()) && cameraCapabilities.isSupportPortraitLighting()) {
    ...
    static void applyWaterMark(Builder builder, int i, CameraCapabilities cameraCapabilities, CameraConfigs cameraConfigs) {
        if (builder != null && i == 3 && cameraCapabilities.isSupportWatermark() && !DataRepository.dataItemFeature().fP()) {
            ...
            if (DataRepository.dataItemFeature().fQ() && cameraCapabilities.isSupportCustomWatermark() && join.contains("device")) {
                if (!CameraSettings.getCustomWatermark().equals(CameraSettings.getDefaultWatermarkStr()) || !CameraSettings.isUltraPixelRearOn() || DataRepository.dataItemFeature().gI()) {
                    File file = new File(Util.WATERMARK_STORAGE_DIRECTORY, Util.WATERMARK_FILE_NAME);
                    if (!file.exists()) {
                        Util.generateWatermark2File();
                    }
                    MiCameraCompat.applyCustomWaterMark(builder, file.getAbsolutePath());
                } else {
                    File file2 = new File(Util.WATERMARK_STORAGE_DIRECTORY, Util.WATERMARK_ULTRA_PIXEL_FILE_NAME);
                    if (!file2.exists()) {
                        Util.generateUltraPixelWatermark2File();
                    }
                    MiCameraCompat.applyCustomWaterMark(builder, file2.getAbsolutePath());
                }
            }
