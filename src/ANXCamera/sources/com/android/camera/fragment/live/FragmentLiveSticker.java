...
public class FragmentLiveSticker extends FragmentLiveBase implements OnCtaNoticeClickListener {
    ...
    private static class StickerItemAdapter extends Adapter<StickerItemHolder> {
        Context mContext;
        f mGlideOptions = new f().R(R.drawable.ic_live_sticker_placeholder);
    ...
    private void updateData() {
        new TTLiveStickerResourceRequest(CameraSettings.isLiveStickerInternalChannel() ? "local_test" : MonitorCommonConstants.DEFAULT_AID, MonitorCommonConstants.DEFAULT_AID).execute(!CameraSettings.isLiveStickerInternalChannel(), new ResponseListener() {
    ...
    public void updateStickerList(List<LiveStickerInfo> list) {
        ...
        if (list != null) {
            ...
            if (DataRepository.dataItemGlobal().isTiktokMoreButtonEnabled(Util.isPackageAvailable(getActivity(), PACKAGE_NAME))) {
