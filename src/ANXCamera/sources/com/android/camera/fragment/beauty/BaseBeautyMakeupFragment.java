...
public abstract class BaseBeautyMakeupFragment extends BaseBeautyFragment {
    ...
    private void calcItemWidthAndNeedScroll() {
        ...
        int i = this.mHeaderElement != -1 ? this.mHeaderCustomWidth > 0 ? this.mHeaderCustomWidth : getResources().getDimensionPixelSize(R.dimen.beauty_header_width) + getResources().getDimensionPixelSize(R.dimen.beauty_divider_stroke) : 0;
        int dimensionPixelSize = this.mFooterElement != -1 ? getResources().getDimensionPixelSize(R.dimen.beauty_divider_stroke) + getResources().getDimensionPixelSize(R.dimen.beauty_header_width) : 0;
        int size = this.mItemList.size();
        int i2 = (int) (((float) (this.mTotalWidth - i)) / 4.5f);
        int max = Math.max(((this.mTotalWidth - i) - dimensionPixelSize) / size, i2);
        if (max == i2) {
            this.mNeedScroll = true;
        } else {
            this.mNeedScroll = false;
            max = (((this.mTotalWidth - (this.mItemMargin * 2)) - i) - dimensionPixelSize) / size;
        }
        this.mItemWidth = max;
    }
    ...
    private void initHeaderView() {
        if (this.mHeaderElement != -1) {
            ...
            if (this.mHeaderCustomWidth > 0) {
                marginLayoutParams.width = this.mHeaderCustomWidth;
            }
