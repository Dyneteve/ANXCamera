...
public class CameraSnapPaintRound extends CameraPaintBase {
    ...
    public void draw(Canvas canvas) {
        if (!this.isRecording) {
            ...
            canvas.drawCircle(this.mMiddleX, this.mMiddleY, this.mBaseRadius * this.mRoundingProgress * 0.55f, this.mPaint);
        } else {
    ...
    public void updateRecordValue(float f, boolean z) {
        if (z) {
            if (this.isRoundingCircle) {
                this.mRoundingProgress = 1.0f - (0.35f * f);
            } else {
        ...
        if (this.isRoundingCircle) {
            this.mRoundingProgress = 0.65f + (0.35f * f);
        } else {
