...
public abstract class SurfaceTextureScreenNail implements OnFrameAvailableListener, Rotatable, ScreenNail {
    private static final float MOVIE_SOLID_CROPPED_X = (b.hS() ? 0.9f : 0.8f);
    ...
    private void initializePreviewTexture() {
        ...
        if (b.iB() && !sFrameListener.isAlive()) {
    ...
    public void draw(GLCanvas gLCanvas) {
        ...
        if (!(externalFrameProcessor == null || this.mSurfaceTexture == null)) {
            this.mSurfaceTexture.updateTexImage();
            if (!isAnimationRunning() && externalFrameProcessor.isProcessorReady()) {
                return;
            }
        }
        gLCanvas.clearBuffer();
        if (!this.mIsFullScreen || b.hK() || Util.isNotchDevice || Util.isLongRatioScreen || Util.sIsnotchScreenHidden) {
