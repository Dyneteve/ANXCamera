...
public class ImageSaver implements SaverCallback, ParallelCallback, VideoClipSavingCallback {
    ...
    private void insertMimojiCaptureTask(ParallelTaskData parallelTaskData) {
        ...
        addImage(jpegImageData, parallelTaskData.isNeedThumbnail(), createJpegName, null, System.currentTimeMillis(), null, parallelTaskDataParameter2.getLocation(), width, height, null, jpegRotation, false, false, true, false, false, Util.ALGORITHM_NAME_MIMOJI_CAPTURE, parallelTaskDataParameter2.getPictureInfo(), parallelTaskData.getPreviewThumbnailHash());
    }

    /* JADX WARNING: Removed duplicated region for block: B:15:0x008e  */
    /* JADX WARNING: Removed duplicated region for block: B:36:0x01ad  */
    /* JADX WARNING: Removed duplicated region for block: B:39:0x01b6  */
    /* JADX WARNING: Removed duplicated region for block: B:40:0x01ec  */
    /* JADX WARNING: Removed duplicated region for block: B:43:0x0212  */
    /* JADX WARNING: Removed duplicated region for block: B:44:0x021b  */
    private void insertNormalDualTask(ParallelTaskData parallelTaskData) {
        ...
        if (parallelTaskData.isAdaptiveSnapshotSize()) {
            ...
            sb3.append("insertNormalDualTask: isShot2Gallery = ");
        ...
        sb4.append(Util.createJpegName(System.currentTimeMillis()));
        sb4.append(dataParameter.getSuffix());
        ...
        sb32.append("insertNormalDualTask: isShot2Gallery = ");
        sb32.append(parallelTaskData.isShot2Gallery());
    ...
    private void insertParallelBurstTask(ParallelTaskData parallelTaskData) {
        ...
        byte[] appendExif = Util.appendExif(parallelTaskData.getJpegImageData(), parallelTaskData);
        ...
        int width = dataParameter.getPictureSize().getWidth();
        int height = dataParameter.getPictureSize().getHeight();
        int orientation = Exif.getOrientation(appendExif);
        int jpegRotation = dataParameter.getJpegRotation();
        ...
        byte[] composeMainSubPicture = Util.composeMainSubPicture(appendExif, dataOfTheRegionUnderWatermarks, coordinatesOfTheRegionUnderWatermarks);
    ...
    private void insertParallelDualTask(ParallelTaskData parallelTaskData) {
        ...
        if ((6 == parallelTaskData.getParallelType() || 8 == parallelTaskData.getParallelType() || 7 == parallelTaskData.getParallelType()) && ArcsoftDepthMap.isDepthMapData(parallelTaskData.getPortraitDepthData())) {
            composeMainSubPicture = Util.composeDepthMapPicture(appendExif, parallelTaskData.getPortraitDepthData(), parallelTaskData.getPortraitRawData(), dataOfTheRegionUnderWatermarks, coordinatesOfTheRegionUnderWatermarks, dataParameter.isHasDualWaterMark(), dataParameter.isHasFrontWaterMark(), dataParameter.getLightingPattern(), dataParameter.getTimeWaterMarkString(), dataParameter.getOutputSize().getWidth(), dataParameter.getOutputSize().getHeight(), dataParameter.isMirror(), dataParameter.isBokehFrontCamera(), dataParameter.getPictureInfo());
        } else if (parallelTaskData.isLiveShotTask()) {
            composeMainSubPicture = Util.composeLiveShotPicture(appendExif, dataParameter.getOutputSize().getWidth(), dataParameter.getOutputSize().getHeight(), parallelTaskData.getMicroVideoData(), parallelTaskData.getCoverFrameTimestamp(), dataParameter.isHasDualWaterMark(), dataParameter.isHasFrontWaterMark(), dataParameter.getTimeWaterMarkString(), dataOfTheRegionUnderWatermarks, coordinatesOfTheRegionUnderWatermarks);
        } else {
            composeMainSubPicture = Util.composeMainSubPicture(appendExif, dataOfTheRegionUnderWatermarks, coordinatesOfTheRegionUnderWatermarks);
        }
    ...
    private void insertSingleTask(ParallelTaskData parallelTaskData) {
        ...
        sb6.append("insertSingleTask: isShot2Gallery = ");
        sb6.append(parallelTaskData.isShot2Gallery());
    ...
    private void storeJpegData(ParallelTaskData parallelTaskData, int i, int i2) {
        ...
        Bitmap createBitmap = Thumbnail.createBitmap(parallelTaskData.getJpegImageData(), i2 + (360 - dataParameter.getShootOrientation()) + (imageSaverCallback == null ? 0 : imageSaverCallback.getDisplayRotation()), false, Integer.highestOneBit((int) Math.floor(((double) i) / ((double) dataParameter.getPreviewSize().getWidth()))));
