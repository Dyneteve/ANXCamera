...
public class ConfigChangeImpl implements ConfigChanges {
    ...
    private void configMacroMode() {
        ...
        if (DataRepository.dataItemFeature().hu()) {
            OnOpticalZoomRatioChangeProtocol onOpticalZoomRatioChangeProtocol = (OnOpticalZoomRatioChangeProtocol) ModeCoordinatorImpl.getInstance().getAttachProtocol(203);
            if (onOpticalZoomRatioChangeProtocol != null) {
                onOpticalZoomRatioChangeProtocol.onOpticalZoomRatioUpdate(bool.booleanValue());
            }
    ...
    private void configSwitchHandGesture() {
        if (DataRepository.dataItemRunning().supportHandGesture() && getBaseModule() != null) {
    ...
    public void configLiveShotSwitch(int i) {
        if (isAlive()) {
            ...
            if (baseModule != null && baseModule.isFrameAvailable()) {
                if ((baseModule.getModuleIndex() == 163 || baseModule.getModuleIndex() == 165) && DataRepository.dataItemFeature().gq()) {
                    TopAlert topAlert = (TopAlert) ModeCoordinatorImpl.getInstance().getAttachProtocol(172);
                    if (topAlert != null) {
                        Camera2Module camera2Module = (Camera2Module) baseModule;
                        if (i != 1) {
            ...
            if (baseModule != null && baseModule.isFrameAvailable()) {
                ...
                switch (c) {
                    ...
                    case 5:
                        ...
                        if (moduleIndex == 165 || moduleIndex == 163) {
                            closeMutexElement(SupportedConfigFactory.CLOSE_BY_RATIO, 206);
                            DataRepository.dataItemRunning().setRecordingClosedElements(this.mRecordingClosedElements);
                            break;
                        }
                    default:
    ...
    public void reCheckFrontBokenTip() {
        if (DataRepository.dataItemFeature().hl() && ((TopAlert) ModeCoordinatorImpl.getInstance().getAttachProtocol(172)) != null && "on".equals(DataRepository.dataItemConfig().getComponentBokeh().getComponentValue(getBaseModule().getModuleIndex()))) {
            updateTipMessage(4, R.string.bokeh_use_hint, 2);
        }
    }
