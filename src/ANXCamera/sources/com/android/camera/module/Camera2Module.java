...
public class Camera2Module extends BaseModule implements Listener, CameraAction, CameraModuleSpecial, FilterProtocol, TopConfigProtocol, ObjectViewListener, BeautyBodySlimCountCallback, CameraMetaDataCallback, CameraPreviewCallback, FaceDetectionCallback, FocusCallback, HDRCheckerCallback, LivePhotoResultCallback, PictureCallback, ScreenLightCallback, UltraWideCheckCallback {
    ...
    private SensorStateListener mSensorStateListener = new SensorStateListener() {
        ...
        public void onDeviceLieChanged(boolean z) {
            if (!Camera2Module.this.mPaused && Camera2Module.this.mIsShowLyingDirectHintStatus != Camera2Module.this.mOrientationCompensation + (z ? 1 : 0)) {
                Camera2Module.this.mIsShowLyingDirectHintStatus = Camera2Module.this.mOrientationCompensation + z;
    ...
    private static class DBRunnable implements Runnable {
        ParallelTaskData mData;
        WeakReference<ImageSaver> mImageSaver;
        boolean mInsertMediaStore;

        public DBRunnable(ParallelTaskData parallelTaskData, boolean z, ImageSaver imageSaver) {
            this.mData = parallelTaskData;
            this.mInsertMediaStore = z;
            this.mImageSaver = new WeakReference<>(imageSaver);
        }

        public void run() {
            if (DbRepository.dbItemSaveTask().getItemByPath(this.mData.getSavePath()) == null) {
                SaveTask saveTask = (SaveTask) DbRepository.dbItemSaveTask().generateItem(System.currentTimeMillis());
                saveTask.setPath(this.mData.getSavePath());
                DbRepository.dbItemSaveTask().endItemAndInsert(saveTask, 0);
                String access$300 = Camera2Module.TAG;
                StringBuilder sb = new StringBuilder();
                sb.append("endItemAndInsert end getSavePath:");
                sb.append(this.mData.getSavePath());
                Log.d(access$300, sb.toString());
            }
            if (this.mInsertMediaStore && this.mImageSaver.get() != null) {
                ((ImageSaver) this.mImageSaver.get()).onParallelProcessFinish(this.mData);
            }
        }
    }
            ...
            if (!Camera2Module.this.mPaused && bArr != null && Camera2Module.this.mReceivedJpegCallbackNum < Camera2Module.this.mTotalJpegCallbackNum && Camera2Module.this.mMultiSnapStatus) {
                ...
                if (!Camera2Module.this.mActivity.getImageSaver().isSaveQueueFull()) {
                    ...
                    if (!Camera2Module.this.mMutexModePicker.isUbiFocus() || Camera2Module.this.mReceivedJpegCallbackNum != Camera2Module.this.mTotalJpegCallbackNum) {
                        Camera2Module.this.mActivity.getImageSaver().addImage(bArr, (Camera2Module.this.mReceivedJpegCallbackNum != 1 || Camera2Module.this.mMultiSnapStopRequest) && (Camera2Module.this.mReceivedJpegCallbackNum == Camera2Module.this.mTotalJpegCallbackNum || Camera2Module.this.mMultiSnapStopRequest || this.mDropped), burstShotTitle, null, System.currentTimeMillis(), null, this.mLocation, i, i2, null, orientation, z, z2, true, false, false, null, Camera2Module.this.getPictureInfo(), -1);
                        this.mDropped = false;
                    }
                } else {
                    String access$300 = Camera2Module.TAG;
            ...
            if (this.mParallelParameter == null) {
                if (CameraSettings.isAgeGenderAndMagicMirrorWaterOpen()) {
                    ...
                    if (faceWaterMarkInfos != null && !faceWaterMarkInfos.isEmpty()) {
                        ...
                        this.mParallelParameter = new Builder(Camera2Module.this.mPreviewSize.toSizeObject(), cameraSize.toSizeObject(), Camera2Module.this.mOutPutSize != null ? cameraSize.toSizeObject() : Camera2Module.this.mOutPutSize.toSizeObject()).setHasDualWaterMark(CameraSettings.isDualCameraWaterMarkOpen()).setMirror(Camera2Module.this.isFrontMirror()).setLightingPattern(CameraSettings.getPortraitLightingPattern()).setFilterId(EffectController.getInstance().getEffectForSaving(false)).setOrientation(-1 != Camera2Module.this.mOrientation ? 0 : Camera2Module.this.mOrientation).setJpegRotation(Camera2Module.this.mJpegRotation).setShootRotation((CameraSettings.isGradienterOn() || Camera2Module.this.mShootRotation != -1.0f) ? Camera2Module.this.mShootRotation : 0.0f).setShootOrientation(Camera2Module.this.mShootOrientation).setLocation(Camera2Module.this.mLocation == null ? new Location(Camera2Module.this.mLocation) : null).setTimeWaterMarkString(!CameraSettings.isTimeWaterMarkOpen() ? Util.getTimeWatermark() : null).setFaceWaterMarkList(list).setAgeGenderAndMagicMirrorWater(CameraSettings.isAgeGenderAndMagicMirrorWaterOpen()).setFrontCamera(Camera2Module.this.isFrontCamera()).setBokehFrontCamera(Camera2Module.this.isBokehFrontCamera()).setAlgorithmName(Camera2Module.this.mAlgorithmName).setPictureInfo(Camera2Module.this.getPictureInfo()).setSuffix(Camera2Module.this.getSuffix()).setGradienterOn(Camera2Module.this.mIsGradienterOn).setTiltShiftMode(Camera2Module.getTiltShiftMode()).setSaveGroupshotPrimitive(false).setDeviceWatermarkParam(Camera2Module.this.getDeviceWaterMarkParam()).setJpegQuality(BaseModule.getJpegQuality(true)).build();
                    }
                }
                list = null;
                this.mParallelParameter = new Builder(Camera2Module.this.mPreviewSize.toSizeObject(), cameraSize.toSizeObject(), Camera2Module.this.mOutPutSize != null ? cameraSize.toSizeObject() : Camera2Module.this.mOutPutSize.toSizeObject()).setHasDualWaterMark(CameraSettings.isDualCameraWaterMarkOpen()).setMirror(Camera2Module.this.isFrontMirror()).setLightingPattern(CameraSettings.getPortraitLightingPattern()).setFilterId(EffectController.getInstance().getEffectForSaving(false)).setOrientation(-1 != Camera2Module.this.mOrientation ? 0 : Camera2Module.this.mOrientation).setJpegRotation(Camera2Module.this.mJpegRotation).setShootRotation((CameraSettings.isGradienterOn() || Camera2Module.this.mShootRotation != -1.0f) ? Camera2Module.this.mShootRotation : 0.0f).setShootOrientation(Camera2Module.this.mShootOrientation).setLocation(Camera2Module.this.mLocation == null ? new Location(Camera2Module.this.mLocation) : null).setTimeWaterMarkString(!CameraSettings.isTimeWaterMarkOpen() ? Util.getTimeWatermark() : null).setFaceWaterMarkList(list).setAgeGenderAndMagicMirrorWater(CameraSettings.isAgeGenderAndMagicMirrorWaterOpen()).setFrontCamera(Camera2Module.this.isFrontCamera()).setBokehFrontCamera(Camera2Module.this.isBokehFrontCamera()).setAlgorithmName(Camera2Module.this.mAlgorithmName).setPictureInfo(Camera2Module.this.getPictureInfo()).setSuffix(Camera2Module.this.getSuffix()).setGradienterOn(Camera2Module.this.mIsGradienterOn).setTiltShiftMode(Camera2Module.getTiltShiftMode()).setSaveGroupshotPrimitive(false).setDeviceWatermarkParam(Camera2Module.this.getDeviceWaterMarkParam()).setJpegQuality(BaseModule.getJpegQuality(true)).build();
    ...
    public void beginParallelProcess(ParallelTaskData parallelTaskData, boolean z) {
        ...
        if (z) {
            insertParallelTaskData(parallelTaskData, false);
        }
    ...
    private void checkMoreFrameCaptureLockAFAE(boolean z) {
        if (DataRepository.dataItemFeature().hz()) {
            if ((ModuleManager.isSuperNightScene() || this.mMutexModePicker.isHdr()) && !is3ALocked()) {
                if (z) {
  ...
  mth insns count: 121
     ...
     */
    /* JADX WARNING: Unknown top exception splitter block from list: {B:56:0x010c=Splitter:B:56:0x010c, B:51:0x00fd=Splitter:B:51:0x00fd} */
            ...
            if (this.mCropValue != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        e = e5;
                        r28 = openOutputStream;
                        try {
                            Log.e(TAG, "Exception when doAttach: ", e);
                            this.mActivity.finish();
                            Util.closeSilently(r28);
                        } catch (Throwable th3) {
                            th = th3;
                            r27 = r28;
                            this.mActivity.finish();
                            Util.closeSilently(r27);
                            throw th;
                        }
                    } catch (Throwable th4) {
                        th = th4;
                        r27 = openOutputStream;
                        this.mActivity.finish();
                        Util.closeSilently(r27);
                        throw th;
                    }
    ...
    private boolean enableFrontMFNR() {
        return b.jk() && (this.mOperatingMode == 32773 || (DataRepository.dataItemFeature().gS() && this.mOperatingMode == 36865));
    }
    ...
    private boolean enablePreviewAsThumbnail() {
        ...
        if (this.mModuleIndex == 167 || this.mModuleIndex == 173) {
            return false;
        }
        if (this.mModuleIndex == 175) {
            return DataRepository.dataItemFeature().gG();
        }
        if (CameraSettings.showGenderAge() || CameraSettings.isMagicMirrorOn() || CameraSettings.isTiltShiftOn() || CameraSettings.isUltraPixelOn()) {
            return false;
        }
    ...
    public DeviceWatermarkParam getDeviceWaterMarkParam() {
        ...
        if (isDualCameraWaterMarkOpen) {
            ...
            f2 = 0.0f;
            f = 0.0f;
            DeviceWatermarkParam deviceWatermarkParam = new DeviceWatermarkParam(isDualCameraWaterMarkOpen, isFrontCameraWaterMarkOpen, CameraSettings.isUltraPixelRearOn(), CameraSettings.getDualCameraWaterMarkFilePathVendor(), f3, f2, f);
            return deviceWatermarkParam;
        }
        f = resourceFloat3;
        f3 = resourceFloat;
        f2 = resourceFloat2;
        DeviceWatermarkParam deviceWatermarkParam2 = new DeviceWatermarkParam(isDualCameraWaterMarkOpen, isFrontCameraWaterMarkOpen, CameraSettings.isUltraPixelRearOn(), CameraSettings.getDualCameraWaterMarkFilePathVendor(), f3, f2, f);
    ...
    private void insertParallelTaskData(ParallelTaskData parallelTaskData, boolean z) {
        if (parallelTaskData != null && !TextUtils.isEmpty(parallelTaskData.getSavePath())) {
            GlobalConstant.sThumbnailSaverScheduler.scheduleDirect(new DBRunnable(parallelTaskData, z, this.mActivity == null ? null : this.mActivity.getImageSaver()));
        }
    }
    ...
    private boolean isCannotGotoGallery() {
        return this.mPaused || this.isZooming || isKeptBitmapTexture() || this.mMultiSnapStatus || getCameraState() == 0 || isParallelQueueFull() || isInCountDown();
    }

    private boolean isCurrentRawDomainBasedSuperNight() {
        return this.mModuleIndex == 173 && DataRepository.dataItemFeature().hx();
    }
    ...
    private boolean isEnableQcfaForAlgoUp() {
        ...
        if (CameraSettings.isUltraPixelOn() && (b.sL || b.sM || b.sO)) {
            z = true;
        }
        return z;
    }
    ...
    private boolean isSensorRawStreamRequired() {
        ...
        if (i == 167) {
            return CameraSettings.isEnableDNG();
        }
        if (i != 173) {
            return false;
        }
        return DataRepository.dataItemFeature().hx();
    }
    ...
    private boolean isTriggerQcfaModeChange(boolean z, boolean z2) {
        ...
        if ((this.mModuleIndex == 171 && isBokehFrontCamera()) || DataRepository.dataItemFeature().gK() > 0) {
            return false;
        }
        if (z) {
            if (!mIsBeautyFrontOn) {
    ...
    public static /* synthetic */ void lambda$onPause$8(Camera2Module camera2Module) {
    ...
    public static /* synthetic */ void lambda$onUltraWideChanged$11(Camera2Module camera2Module, boolean z, int i) {
        ...
        if (bottomPopupTips != null) {
            int i2 = 2131297148;
            if (!camera2Module.isZoomRatioBetweenUltraAndWide() || (!bottomPopupTips.containTips(R.string.ultra_wide_recommend_tip_hint) && !bottomPopupTips.containTips(R.string.ultra_wide_recommend_tip_hint_sat))) {
                if (z) {
    ...
    public static /* synthetic */ void lambda$startNormalCapture$4(Camera2Module camera2Module, boolean z) {
        ...
        sb.append(z);
        Log.d(str, sb.toString());
        if (DataRepository.dataItemFeature().ha()) {
    ...
    private void prepareNormalCapture() {
        ...
        camera2Proxy.setShotSavePath(Storage.generateFilepath(sb2.toString()), !this.mMultiSnapStatus && (this.mEnableParallelSession || this.mEnableShot2Gallery));
        if (this.mModuleIndex == 163 || this.mModuleIndex == 165 || this.mModuleIndex == 171 || this.mModuleIndex == 175) {
            ...
            boolean ha = DataRepository.dataItemFeature().ha();
            if (z2 || ha) {
                z = true;
    ...
    private void realConsumeAiSceneResult(int i, boolean z) {
        ...
        if (!isDoingAction() && isAlive() && !this.mActivity.isActivityPaused()) {
            if (!z || !this.isResetFromMutex) {
                ...
                backlightHandle(topAlert2, configChanges);
    ...
    private void resetAsdSceneInHdrOrFlashChange() {
        if (b.ji() && isFrontCamera() && this.mCurrentAsdScene != -1 && this.mCurrentAsdScene == 9) {
            this.mHandler.post(new Runnable() {
                public void run() {
    ...
    private void setAiSceneEffect(int i) {
        ...
        sb.append(i);
        Log.d(str, sb.toString());
        if (!DataRepository.dataItemFeature().fU() || !CameraSettings.isBackCamera() || i != 25) {
    ...
    private void setWaterMark() {
        if (this.mMultiSnapStatus || this.mModuleIndex == 165 || CameraSettings.isGradienterOn() || CameraSettings.getShaderEffect() != FilterInfo.FILTER_ID_NONE || this.mHasAiSceneFilterEffect || CameraSettings.isTiltShiftOn() || DataRepository.dataItemFeature().fP()) {
    ...
    private boolean shouldApplyUltraWideLDC() {
        ...
        if (CameraSettings.isMacroModeEnabled(this.mModuleIndex) && !DataRepository.dataItemFeature().hu()) {
            return true;
        }
    ...
    public void startNormalCapture(int i) {
        ...
        if (!CameraSettings.isGroupShotOn() || isParallelSessionEnable()) {
            ...
            if (isFrontCamera() && DataRepository.dataItemFeature().ha() && 32775 != this.mOperatingMode) {
    ...
    private void unlockAEAF() {
        ...
        if (this.mAeLockSupported) {
            this.mCamera2Device.unlockExposure();
            if (!DataRepository.dataItemFeature().ht() && this.mUltraWideAELocked) {
    ...
    private void updateAlgorithmName() {
        String str = !b.jq() ? this.mCamera2Device.isBokehEnabled() ? DataRepository.dataItemFeature().gD() > 0 ? Util.ALGORITHM_NAME_SOFT_PORTRAIT_ENCRYPTED : Util.ALGORITHM_NAME_SOFT_PORTRAIT : isPortraitMode() ? "portrait" : this.mMutexModePicker.getAlgorithmName() : null;
    ...
    private void updateBokeh() {
        boolean z = false;
        this.mCamera2Device.setMiBokeh(isFrontCamera() && !isBokehFrontCamera() && ((ModuleManager.isPortraitModule() && DataRepository.dataItemFeature().hj()) || (DataRepository.dataItemFeature().hl() && "on".equals(DataRepository.dataItemConfig().getComponentBokeh().getComponentValue(this.mModuleIndex)))));
    ...
    private void updateDecodePreview() {
        if (this.mIsGoogleLensAvailable || scanQRCodeEnabled() || b.gL()) {
            String str = TAG;
            StringBuilder sb = new StringBuilder();
    ...
    private void updateEnablePreviewThumbnail(boolean z) {
        if (this.mEnableParallelSession || this.mEnableShot2Gallery || z || (this.mReceivedJpegCallbackNum == 0 && enablePreviewAsThumbnail())) {
    ...
    private void updateEyeLight() {
        if (isFrontCamera() && DataRepository.dataItemFeature().gi()) {
    ...
    private void updateLiveShot() {
        if (DataRepository.dataItemFeature().gq() && this.mModuleIndex == 163) {
            if (CameraSettings.isLiveShotOn()) {
                startLiveShot();
            } else {
                stopLiveShot(false);
    ...
    private void updateMfnr(boolean z) {
        boolean z2 = false;
        if (!isUseSwMfnr() && z && this.mModuleIndex != 167 && ((!b.sE || isDualCamera()) && this.mMutexModePicker.isNormal() && !CameraSettings.isGroupShotOn() && ((!isFrontCamera() || enableFrontMFNR()) && (DataRepository.dataItemFeature().fE() || getZoomRatio() == 1.0f || isUltraWideBackCamera() || isZoomRatioBetweenUltraAndWide())))) {
            z2 = true;
        }
    ...
    private void updatePictureAndPreviewSize() {
        ...
        if (isSensorRawStreamRequired()) {
            this.mSensorRawImageSize = getBestPictureSize(this.mCameraCapabilities.getSupportedOutputSize(32));
            String str = TAG;
            StringBuilder sb = new StringBuilder();
        ...
        if (this.mEnableParallelSession) {
            ...
            if (this.mModuleIndex == 165) {
                ...
                if (this.mOutPutSize.getWidth() == 0) {
                    int i2 = this.mPictureSize.width > this.mPictureSize.height ? this.mPictureSize.height : this.mPictureSize.width;
                    throw new RuntimeException(String.format(Locale.ENGLISH, "size %dx%d is not supported!", new Object[]{Integer.valueOf(i2), Integer.valueOf(i2)}));
                }
            } else {
    ...
    private void updateShotDetermine() {
        ...
        if (this.mModuleIndex == 171) {
            if (!isBackCamera()) {
                z = DataRepository.dataItemFeature().fJ();
            } else if (b.jj() || DataRepository.dataItemFeature().fI()) {
                z = true;
            }
            if (!this.mIsImageCaptureIntent) {
                i = z ? -3 : -2;
                ...
                if (this.mEnableParallelSession || !DataRepository.dataItemFeature().gG() || !enablePreviewAsThumbnail()) {
                    z2 = false;
                }
                ...
                if (!(i3 == 163 || i3 == 165)) {
                    if (i3 != 171) {
                        ...
                        } else if (isCurrentRawDomainBasedSuperNight()) {
                            i2 = 10;
                        }
    ...
    private void updateSuperNight() {
        this.mCamera2Device.setSuperNight(this.mModuleIndex == 173);
    }
    ...
    public void closeCamera() {
        ...
        synchronized (this.mCameraDeviceLock) {
            if (this.mCamera2Device != null) {
                ...
                this.mCamera2Device.setASD(false);
                if (scanQRCodeEnabled() || b.gL() || this.mIsGoogleLensAvailable) {
    ...
    public int getMutexHdrMode(String str) {
        ...
        return (!b.hW() || !"live".equals(str)) ? 0 : 2;
    }
    ...
    public int getOperatingMode() {
        ...
        if (!isParallelSessionEnable()) {
            ...
            if (isFrontCamera) {
                ...
                if (!isPortraitMode() || !DataRepository.dataItemFeature().fJ()) {
                    if (!isPortraitMode() || !isBokehFrontCamera()) {
                        if (!this.mCameraCapabilities.isSupportedQcfa() || mIsBeautyFrontOn || !"off".equals(DataRepository.dataItemConfig().getComponentHdr().getComponentValue(this.mModuleIndex)) || DataRepository.dataItemFeature().gK() > 0) {
                            i = 32773;
                            i2 = i;
    ...
    public ParallelTaskData onCaptureStart(ParallelTaskData parallelTaskData, CameraSize cameraSize, boolean z) {
        ...
        if (CameraSettings.isAgeGenderAndMagicMirrorWaterOpen()) {
            ...
            if (faceWaterMarkInfos != null && !faceWaterMarkInfos.isEmpty()) {
                ...
                Log.d(str2, sb.toString());
                Builder location = new Builder(this.mPreviewSize.toSizeObject(), cameraSize.toSizeObject(), this.mOutPutSize != null ? cameraSize.toSizeObject() : this.mOutPutSize.toSizeObject()).setHasDualWaterMark(CameraSettings.isDualCameraWaterMarkOpen()).setMirror(isFrontMirror()).setLightingPattern(CameraSettings.getPortraitLightingPattern()).setFilterId(EffectController.getInstance().getEffectForSaving(false)).setOrientation(-1 != this.mOrientation ? 0 : this.mOrientation).setJpegRotation(this.mJpegRotation).setShootRotation((CameraSettings.isGradienterOn() || this.mShootRotation != -1.0f) ? this.mShootRotation : 0.0f).setShootOrientation(this.mShootOrientation).setLocation(this.mLocation);
                if (CameraSettings.isTimeWaterMarkOpen()) {
                ...
                if (b.isMTKPlatform()) {
                    ...
                    cameraCharacteristics.putFloatArray(CameraCharacteristics.LENS_INFO_AVAILABLE_FOCAL_LENGTHS.toString(), (float[]) cameraCharacteristics2.get(CameraCharacteristics.LENS_INFO_AVAILABLE_FOCAL_LENGTHS));
                    cameraCharacteristics.putFloatArray(CameraCharacteristics.LENS_INFO_AVAILABLE_APERTURES.toString(), (float[]) cameraCharacteristics2.get(CameraCharacteristics.LENS_INFO_AVAILABLE_APERTURES));
                }
        ...
        Builder location2 = new Builder(this.mPreviewSize.toSizeObject(), cameraSize.toSizeObject(), this.mOutPutSize != null ? cameraSize.toSizeObject() : this.mOutPutSize.toSizeObject()).setHasDualWaterMark(CameraSettings.isDualCameraWaterMarkOpen()).setMirror(isFrontMirror()).setLightingPattern(CameraSettings.getPortraitLightingPattern()).setFilterId(EffectController.getInstance().getEffectForSaving(false)).setOrientation(-1 != this.mOrientation ? 0 : this.mOrientation).setJpegRotation(this.mJpegRotation).setShootRotation((CameraSettings.isGradienterOn() || this.mShootRotation != -1.0f) ? this.mShootRotation : 0.0f).setShootOrientation(this.mShootOrientation).setLocation(this.mLocation);
        if (CameraSettings.isTimeWaterMarkOpen()) {
        }
    ...
    public void onFaceDetected(CameraHardwareFace[] cameraHardwareFaceArr, FaceAnalyzeInfo faceAnalyzeInfo) {
        if (isAlive() && this.mActivity.getCameraScreenNail().getFrameAvailableFlag() && cameraHardwareFaceArr != null) {
            ...
            if (!b.hZ() || cameraHardwareFaceArr.length <= 0 || cameraHardwareFaceArr[0].faceType != 64206) {
            ...
            switch (focusTask.getFocusTrigger()) {
                case 1:
                    ...
                    if (focusTask.isSuccess() && this.m3ALocked) {
                        if (!DataRepository.dataItemFeature().ht() && isZoomRatioBetweenUltraAndWide() && this.mUltraCameraCapabilities != null) {
    ...
    public boolean onInterceptZoomingEvent(float f, float f2, int i) {
        ...
        if (DataRepository.dataItemFeature().ht()) {
            if (this.m3ALocked) {
                unlockAEAF();
                if (this.mFocusManager != null) {
    ...
    public void onPause() {
        ...
        if (Util.isGlobalVersion() && !b.gE()) {
            this.mActivity.runOnUiThread(new Runnable() {
                public final void run() {
                    Camera2Module.lambda$onPause$8(Camera2Module.this);
                }
            });
        }
        if (this.mHandler != null) {
    ...
    public void onPreviewMetaDataUpdate(CaptureResult captureResult) {
        if (captureResult != null) {
            ...
            boolean z = false;
            if (DataRepository.dataItemFeature().gS() || !isFrontCamera()) {
    ...
    public void onPreviewPixelsRead(byte[] bArr, int i, int i2) {
        ...
        if (this.mEnableParallelSession || this.mEnableShot2Gallery) {
            if (this.mIsGradienterOn) {
                ...
                createBitmap = (this.mOrientation == 90 || this.mOrientation == 270) ? Util.flipBitmap(createBitmap, 1) : Util.flipBitmap(createBitmap, 0);
                z = false;
                z2 = false;
            } else {
                z2 = z3;
            ...
            bArr2 = Util.getBitmapData(createBitmap, 87);
        } else {
            z2 = z3;
            z = false;
            bArr2 = null;
        ...
        if (isFrontCamera() && b.ja() && i3 % 180 == 0) {
            i3 = 0;
        }
        synchronized (this.mCameraDeviceLock) {
            if (isAlive()) {
                if (isDeviceAlive()) {
                    ...
                    sb.append(bArr2);
                    Log.d(str, sb.toString());
                    if (bArr2 != null && (this.mEnableParallelSession || this.mEnableShot2Gallery)) {
                        ParallelTaskData parallelTaskData = new ParallelTaskData(System.currentTimeMillis(), -1, this.mCamera2Device.getParallelShotSavePath());
                        ...
                        insertParallelTaskData(parallelTaskData, true);
                    }
                }
            }
    ...
    public void onThumbnailClicked(View view) {
        if (!this.mEnableShot2Gallery && isDoingAction()) {
            Log.d(TAG, "onThumbnailClicked: DoingAction..");
        } else if (!this.mEnableShot2Gallery || !isCannotGotoGallery()) {
    ...
    public boolean scanQRCodeEnabled() {
        return CameraSettings.isScanQRCode(this.mActivity) && this.mModuleIndex == 163 && !this.mIsImageCaptureIntent && CameraSettings.isBackCamera() && !this.mMultiSnapStatus && !CameraSettings.isStereoModeOn() && !CameraSettings.isPortraitModeBackOn() && (!DataRepository.dataItemFeature().gT() || !CameraSettings.isUltraPixelOn()) && !CameraSettings.isUltraWideConfigOpen(this.mModuleIndex) && !CameraSettings.isMacroModeEnabled(this.mModuleIndex);
    }
    ...
    public void startPreview() {
        if (this.mCamera2Device != null) {
            ...
            boolean z = this.mIsGoogleLensAvailable || scanQRCodeEnabled || supportHandGesture;
            if (this.mIsGoogleLensAvailable) {
            ...
            this.mCamera2Device.startPreviewSession(surfaceTexture != null ? new Surface(surfaceTexture) : null, z, isSensorRawStreamRequired(), isCurrentRawDomainBasedSuperNight(), getOperatingMode(), this.mEnableParallelSession, this);
        }
    }
    ...
    public void updateManualEvAdjust() {
        if (this.mModuleIndex == 167) {
            ...
            Log.d(str, sb.toString());
            boolean z = b.jr() ? getString(R.string.pref_camera_exposuretime_default).equals(manualValue) : getString(R.string.pref_camera_iso_default).equals(manualValue2) || getString(R.string.pref_camera_exposuretime_default).equals(manualValue);
            if (this.mHandler != null) {
