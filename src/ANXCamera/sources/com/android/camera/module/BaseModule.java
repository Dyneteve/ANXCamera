...
public abstract class BaseModule implements MutexCallBack, Module, EvChangedProtocol, StandaloneMacroProtocol, UltraWideProtocol, ZoomProtocol, ExposureViewListener, CameraMetaDataCallback, Consumer<int[]> {
    ...
    protected static String getColorEffectKey() {
        return b.hD() ? "pref_camera_shader_coloreffect_key" : CameraSettings.KEY_COLOR_EFFECT;
    }
    ...
    public void initByCapability(CameraCapabilities cameraCapabilities) {
        ...
        boolean z = false;
        this.mAELockOnlySupported = DataRepository.dataItemFeature().gV() && !this.mFocusAreaSupported && this.mMeteringAreaSupported && this.mAeLockSupported;
    ...
    public void initializeZoomRangeFromCapabilities() {
        ...
        if (isBackCamera()) {
                    ...
                    if (i != 174) {
                        switch (i) {
                            ...
                            case 162:
                                ...
                                } else if (DataRepository.dataItemFeature().hu() && isStandaloneMacroCamera()) {
            ...
            } else if (CameraSettings.isMacroModeEnabled(this.mModuleIndex) && DataRepository.dataItemFeature().hu()) {
                setMinZoomRatio(HybridZoomingSystem.getMinimumStandaloneOpticalZoomRatio());
                setMaxZoomRatio(HybridZoomingSystem.getMaximumStandaloneOpticalZoomRatio());
            } else if (CameraSettings.isMacroModeEnabled(this.mModuleIndex)) {
                setMinZoomRatio(HybridZoomingSystem.getMinimumOpticalZoomRatio());
    ...
    public void initializeZoomRatio() {
        ...
        if (i != 165) {
            ...
            if (!CameraSettings.isUltraWideConfigOpen(this.mModuleIndex)) {
                if (CameraSettings.isAutoZoomEnabled(this.mModuleIndex)) {
                    ...
                    sb3.append(HybridZoomingSystem.FLOAT_ZOOM_RATIO_ULTR);
        ...
        if (!CameraSettings.isUltraWideConfigOpen(this.mModuleIndex)) {
            if (CameraSettings.isMacroModeEnabled(this.mModuleIndex) && DataRepository.dataItemFeature().hu()) {
                ...
                sb5.append(HybridZoomingSystem.sDefaultStandaloneMacroOpticalZoomRatio);
                Log.d(str5, sb5.toString());
                setZoomRatio(HybridZoomingSystem.sDefaultStandaloneMacroOpticalZoomRatio);
            } else if (CameraSettings.isMacroModeEnabled(this.mModuleIndex) && !DataRepository.dataItemFeature().hu()) {
                String str6 = TAG;
                StringBuilder sb6 = new StringBuilder();
                sb6.append("resetZoomRatio(): set zoom ratio to ");
                sb6.append(HybridZoomingSystem.FLOAT_ZOOM_RATIO_ULTR);
                Log.d(str6, sb6.toString());
                setZoomRatio(HybridZoomingSystem.FLOAT_ZOOM_RATIO_ULTR);
            } else if (CameraSettings.isSupportedOpticalZoom() || CameraSettings.isUltraPixelRearOn()) {
                Log.d(TAG, "resetZoomRatio(): set zoom ratio to 1.0");
                setZoomRatio(1.0f);
            } else {
                ...
                sb7.append("resetZoomRatio(): set zoom ratio to ");
                sb7.append(zoomRatioHistory2);
    ...
    public boolean isZoomRatioBetweenUltraAndWide(float f) {
        return f >= HybridZoomingSystem.FLOAT_ZOOM_RATIO_ULTR && f < HybridZoomingSystem.FLOAT_ZOOM_RATIO_WIDE;
    }
    ...
    public void onCreate(int i, int i2) {
        ...
        if (DataRepository.dataItemFeature().gy() && CameraSettings.isLensDirtyDetectEnabled()) {
