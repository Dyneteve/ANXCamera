...
public class VideoModule extends VideoBase implements OnErrorListener, OnInfoListener, AutoZoomModuleProtocol, TopConfigProtocol, ObjectViewListener, VideoRecordStateCallback {
    ...
    private long getRecorderMaxFileSize(int i) {
        ...
        if (leftSpace > VIDEO_MAX_SINGLE_FILE_SIZE && DataRepository.dataItemFeature().fX()) {
    ...
    private boolean isEisOn() {
        ...
        if (!b.hA() || z || !CameraSettings.isMacroModeEnabled(this.mModuleIndex)) {
            return z;
        }
        return true;
    }
    ...
    private void onStartRecorderSucceed() {
        ...
        if (HybridZoomingSystem.IS_3_OR_MORE_SAT && this.mModuleIndex != 168 && this.mModuleIndex != 172 && this.mModuleIndex != 170 && !CameraSettings.isMacroModeEnabled(this.mModuleIndex) && !CameraSettings.isAutoZoomEnabled(this.mModuleIndex) && isBackCamera()) {
    ...
    public String postProcessVideo(String str) {
        ...
        boolean z = false;
        try {
            if (VideoInterpolator.doDecodeAndEncodeSync(file.getAbsolutePath(), file2.getAbsolutePath(), DataRepository.dataItemFeature().gp()) && file2.renameTo(file3)) {
                z = true;
            }
            if (isDump960Orig()) {
                StringBuilder sb2 = new StringBuilder();
    ...
    private void setupRecorder(MediaRecorder mediaRecorder) throws FileNotFoundException {
        ...
        if (!DataRepository.dataItemFeature().fX() || (i2 <= 0 && recorderMaxFileSize != VIDEO_MAX_SINGLE_FILE_SIZE)) {
    ...
    private void updateFrontMirror() {
        this.mCamera2Device.setFrontMirror(isFrontCamera() && !b.jx() && DataRepository.dataItemFeature().gf() && CameraSettings.isFrontMirror());
    }

    private void updateHFRDeflicker() {
        if (DataRepository.dataItemFeature().gp() && isFPS960()) {
        ...
        for (int i : iArr) {
            switch (i) {
                ...
                case 5:
                    ...
                    break;
                case 9:
                    updateAntiBanding((!b.hX() || (!isHFRMode() && !isSlowMode())) ? CameraSettings.getAntiBanding() : "0");
                    break;
                case 10:
    ...
    public boolean enableFaceDetection() {
        boolean z = false;
        if (DataRepository.dataItemFeature().gY() && isBackCamera()) {
            return false;
        }
    ...
    public boolean isCameraSwitchingDuringZoomingAllowed() {
        return HybridZoomingSystem.IS_3_OR_MORE_SAT && (this.mModuleIndex == 162 || this.mModuleIndex == 169) && !CameraSettings.isMacroModeEnabled(this.mModuleIndex) && isBackCamera() && !this.mMediaRecorderRecording && !this.mMediaRecorderRecordingPaused;
    }
    ...
    public boolean onGestureTrack(RectF rectF, boolean z) {
        if (this.mInStartingFocusRecording || !isBackCamera() || !b.hZ() || CameraSettings.is4KHigherVideoQuality(this.mQuality) || isCaptureIntent()) {
            return false;
        }
    ...
    public void onZoomingActionStart(int i) {
        ...
        if (!DataRepository.dataItemFeature().hu() || this.mActualCameraId != Camera2DataContainer.getInstance().getStandaloneMacroCameraId()) {
    ...
    public void readVideoPreferences() {
        ...
        if (CameraSettings.VIDEO_SPEED_FAST.equals(this.mSpeed)) {
            ...
            if (isHFRMode() || isSlowMode()) {
                ...
                if (this.mQuality == -1) {
                    ...
                    Size size = 6 == videoQuality ? new Size(1920, ScreenEffect.SCREEN_PAPER_MODE_TWILIGHT_END_DEAULT) : new Size(1280, Util.LIMIT_SURFACE_WIDTH);
        ...
        try {
            ...
            if (!DataRepository.dataItemFeature().gZ() || !CameraSettings.is4KHigherVideoQuality(this.mQuality) || this.mCaptureTimeLapse) {
