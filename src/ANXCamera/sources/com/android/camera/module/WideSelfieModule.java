...
public class WideSelfieModule extends BaseModule implements CameraAction, WideSelfStateCallback, CameraPreviewCallback, FaceDetectionCallback {
    ...
    private static class SaveOutputImageTask extends AsyncTask<Void, Integer, Integer> {
        ...
        private void addImageAsApplication(String str, String str2, byte[] bArr, int i, int i2, int i3) {
            ...
            if (camera != null) {
                ...
                if (uri != null) {
                    ...
                    String str6 = WideSelfieModule.TAG;
        ...
        public Integer doInBackground(Void... voidArr) {
            ...
            if (Storage.isUseDocumentMode()) {
                addImageAsApplication(generateFilepath, this.mFileName, b, this.mWidth, this.mHeight, this.mOrientation);
            } else {
                d.b(generateFilepath, b);
                addImageAsApplication(generateFilepath, this.mFileName, null, this.mWidth, this.mHeight, this.mOrientation);
            }
            CameraStatUtil.trackGeneralInfo(1, false, 176, this.mTriggerMode, true, null, CameraStatUtil.AUTO_OFF);
            CameraStatUtil.trackPictureTakenInWideSelfie(1, this.mStopMode, String.valueOf(i5));
            return null;
        }
    ...
    private List<CameraSize> filterSupportedSize4WideSelfie(List<CameraSize> list) {
        if (list != null) {
            Collections.sort(list, $$Lambda$WideSelfieModule$0LvAwVrD9Fit3bOxPQNPpuUzaKU.INSTANCE);
            for (int i = 0; i < list.size(); i++) {
                CameraSize cameraSize = (CameraSize) list.get(i);
                if (cameraSize.width * cameraSize.height <= 6000000) {
    ...
    static /* synthetic */ int lambda$filterSupportedSize4WideSelfie$0(CameraSize cameraSize, CameraSize cameraSize2) {
        return cameraSize2.width - cameraSize.width;
    }
    ...
    private void updatePictureAndPreviewSize() {
        List supportedOutputSize = this.mCameraCapabilities.getSupportedOutputSize(35);
        PictureSizeManager.initialize(supportedOutputSize, 0, 176, this.mBogusCameraId);
        ...
        } else if (TextUtils.equals(b.rp, "begonia")) {
            bestPictureSize = PictureSizeManager.getBestPictureSize(filterSupportedSize4WideSelfie(supportedOutputSize));
        }
