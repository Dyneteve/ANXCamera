...
public class FunModule extends VideoBase implements FilterProtocol, StickerProtocol {
    ...
    private boolean isEisOn() {
        return isBackCamera() && DataRepository.dataItemFeature().hq() && CameraSettings.isMovieSolidOn();
    }
    ...
    public void stopVideoRecording(boolean z, boolean z2) {
        if (this.mMediaRecorderRecording) {
            ...
            if (this.mCamera2Device != null) {
                CameraStatUtil.trackVideoRecorded(isFrontCamera(), getModuleIndex(), false, CameraSettings.isUltraWideConfigOpen(getModuleIndex()), CameraSettings.VIDEO_MODE_FUN, this.mQuality, this.mCamera2Device.getFlashMode(), 30, 0, this.mBeautyValues, uptimeMillis / 1000);
