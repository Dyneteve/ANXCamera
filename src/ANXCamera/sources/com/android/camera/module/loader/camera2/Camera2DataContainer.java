    ...
    public synchronized int getActualOpenCameraId(int i, int i2) {
        ...
        if (i == 0) {
            if (CameraSettings.isDualCameraEnable() && (CameraSettings.isSupportedOpticalZoom() || CameraSettings.isSupportedPortrait()) && !DataRepository.dataItemGlobal().isForceMainBackCamera()) {
                DataRepository.dataItemConfig().getComponentConfigUltraWide();
                switch (i2) {
                    case 162:
                    case 169:
                        if (CameraSettings.isMacroModeEnabled(i2) && DataRepository.dataItemFeature().hu()) {
                    ...
                    case 174:
                        ...
                        } else if (!DataRepository.dataItemFeature().hu()) {
                            i3 = getUltraWideCameraId();
                            break;
                        } else {
                            i3 = getStandaloneMacroCameraId();
                    ...
                    case 165:
                        if (CameraSettings.isMacroModeEnabled(i2)) {
                            ...
                            int mainBackCameraId = CameraSettings.isUltraPixelOn() ? getMainBackCameraId() : (!CameraSettings.isDualCameraSatEnable() || !b.isSupportedOpticalZoom()) ? CameraSettings.isUltraWideConfigOpen(i2) ? getUltraWideCameraId() : getMainBackCameraId() : (!CameraSettings.isUltraWideConfigOpen(i2) || HybridZoomingSystem.IS_3_OR_MORE_SAT) ? getSATCameraId() : getUltraWideCameraId();
                    ...
                    case 167:
                        if (CameraSettings.isZoomByCameraSwitchingSupported()) {
                            ...
                            i3 = ComponentManuallyDualLens.LENS_WIDE.equals(cameraLensType) ? getMainBackCameraId() : ComponentManuallyDualLens.LENS_TELE.equals(cameraLensType) ? getAuxCameraId() : ComponentManuallyDualLens.LENS_ULTRA.equals(cameraLensType) ? getUltraWideCameraId() : "macro".equals(cameraLensType) ? getStandaloneMacroCameraId() : i;
                            break;
                        }
                    case 171:
