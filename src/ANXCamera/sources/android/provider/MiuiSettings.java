...
public class MiuiSettings {
    ...
    public static class Secure extends android.provider.SystemSettings.Secure {
        ...
        public static Intent buildNewPasswordIntent(String businessKey) {
        ...
        public static void enableUserExperienceProgram(ContentResolver cr, boolean allow) {
            android.provider.Settings.Secure.putInt(cr, UPLOAD_LOG, (int) allow);
        }

        public static boolean isUserExperienceProgramEnable(ContentResolver cr) {
            return android.provider.Settings.Secure.getInt(cr, UPLOAD_LOG, (int) Build.IS_DEVELOPMENT_VERSION) == 1;
        }

        public static void enableUploadDebugLog(ContentResolver cr, boolean allow) {
            android.provider.Settings.Secure.putInt(cr, UPLOAD_DEBUG_LOG, (int) allow);
        }

        public static boolean isUploadDebugLogEnable(ContentResolver cr) {
            return android.provider.Settings.Secure.getInt(cr, UPLOAD_DEBUG_LOG, (int) isUserExperienceProgramEnable(cr)) == 1;
        }

        public static void enableHttpInvokeApp(ContentResolver cr, boolean allow) {
            android.provider.Settings.Secure.putInt(cr, HTTP_INVOKE_APP, (int) allow);
        }
            ...
            if ("enabled_accessibility_services".equals(name)) {
                ...
                for (String oldService : oldServices) {
                    if (!TextUtils.isEmpty(oldService)) {
                        ...
                        int length = newServices.length;
                        int i = 0;
                        while (true) {
                            if (i >= length) {
                                break;
                            } else if (oldService.equals(newServices[i])) {
                                contains = true;
                                break;
                            } else {
                                i++;
                        ...
                        if (!contains) {
                            StringBuilder sb = new StringBuilder();
                            sb.append("disable_");
                            sb.append(oldService.replaceAll("\\W", "_"));
                            android.provider.Settings.Secure.putStringForUser(cr, sb.toString(), cr.getPackageName(), userHandle);
                        }
                    }
                }
                for (String newService : newServices) {
                    if (!TextUtils.isEmpty(newService)) {
                        ...
                        while (true) {
                            if (i2 >= length2) {
                                break;
                            } else if (oldServices[i2].equals(newService)) {
                                contains2 = true;
                                break;
                            } else {
                                i2++;
                        ...
                        if (!contains2) {
                            ...
                            sb2.append("enable_");
                            sb2.append(newService.replaceAll("\\W", "_"));
                            android.provider.Settings.Secure.putStringForUser(cr, sb2.toString(), cr.getPackageName(), userHandle);
                        }
                    }
                }
            }
    ...
    public static final class SilenceMode {
        ...
        public static void reportRingerModeInfo(String type, String mode, String params, long time) {
            ...
            if (DEBUG_MODE) {
                String str = TAG;
                StringBuilder sb = new StringBuilder();
                sb.append("reportRingerModeInfo:");
    ...
    public static class System extends android.provider.SystemSettings.System {
            ...
            if (!TextUtils.isEmpty(ssidList)) {
                ...
                if (ssidArr != null) {
                    for (int i = 0; i < ssidArr.length; i++) {
                        try {
                            ssidSet.add(new String(Base64.decode(ssidArr[i], 2)));
            ...
            if (!TextUtils.isEmpty(macList)) {
                ...
                if (mac != null) {
                    for (int i = 0; i < mac.length; i++) {
                        try {
                            macBlackSet.add(new String(Base64.decode(mac[i], 2)));
                        } catch (IllegalArgumentException e) {
                        }
                    }
                }
        ...
        public static boolean isSimpleMode(Context context) {
            if (android.provider.Settings.System.getInt(context.getContentResolver(), SIMPLE_MODE, (int) (Build.IS_INTERNATIONAL_BUILD ^ 1)) == 1) {
                return true;
            }
            return false;
        }
        ...
        public static Rect getDisplayWindowSizeInSmartCover() {
            ...
            int[] defaultSize = {res.getInteger(285736975), res.getInteger(285736976), res.getInteger(285736977), res.getInteger(285736978)};
            Rect rect = new Rect(defaultSize[0], defaultSize[1], defaultSize[2], defaultSize[3]);
            if (type == null) {
                return rect;
            }
            int[] displaySize = {FeatureParser.getInteger("smartcover_smallwindow_left", defaultSize[0]), FeatureParser.getInteger("smartcover_smallwindow_top", defaultSize[1]), FeatureParser.getInteger("smartcover_smallwindow_right", defaultSize[2]), FeatureParser.getInteger("smartcover_smallwindow_bottom", defaultSize[3])};
