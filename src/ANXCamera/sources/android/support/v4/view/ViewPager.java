...
public class ViewPager extends ViewGroup {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:23:0x0068, code lost:
        if (r8.position == r1.mCurItem) goto L_0x006f;
     */
    ...
    public void smoothScrollTo(int i, int i2, int i3) {
        ...
        int abs = Math.abs(i3);
        int min = Math.min(abs > 0 ? 4 * Math.round(1000.0f * Math.abs(distanceInfluenceForSnapDuration / ((float) abs))) : (int) (((((float) Math.abs(i6)) / ((f * this.mAdapter.getPageWidth(this.mCurItem)) + ((float) this.mPageMargin))) + 1.0f) * 100.0f), MAX_SETTLE_DURATION);
