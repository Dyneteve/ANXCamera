        ...
        try {
            ...
            try {
                ...
                if (address.hostnameVerifier().verify(address.url().host(), sSLSocket2.getSession())) {
                    ...
                    String selectedProtocol = configureSecureSocket.supportsTlsExtensions() ? Platform.get().getSelectedProtocol(sSLSocket2) : str;
                    this.socket = sSLSocket2;
                    this.source = Okio.buffer(Okio.source(this.socket));
                    this.sink = Okio.buffer(Okio.sink(this.socket));
                    this.handshake = handshake2;
                    this.protocol = selectedProtocol != 0 ? Protocol.get(selectedProtocol) : Protocol.HTTP_1_1;
                    if (sSLSocket2 != 0) {
